...
export type ContactFormFieldKey = 'full_name' | 'email' | 'company' | 'budget' | 'message'
...
const formFields: ContactFormField[] = [
	...
	},
	{
		...
		Input: ({hasError, ...props}: ContactInputProps) => {
			return (
				<div className={'w-full mt-1 py-2 justify-between px-3 text-sm grid grid-cols-4 items-center flex-wrap'}>
					<label className={'cursor-pointer flex items-center gap-1.5'} htmlFor="$0">
						<input {...props} type={'radio'} id={'$0'} value={'$0'} name={'budget'} />
						<span className={'text-gray-300'}>$0</span>
					</label>
					<label className={'cursor-pointer flex items-center gap-1.5'} htmlFor="$100">
						<input {...props} type={'radio'} id={'$100'} value={'$100+'}  name={'budget'} />
						<span className={'text-gray-300'}>$100+</span>
					</label>
					<label className={'cursor-pointer flex items-center gap-1.5'} htmlFor="$1000">
						<input {...props} type={'radio'} id={'$1000'} value={'$1000+'} name={'budget'} />
						<span className={'text-gray-300'}>$1000+</span>
					</label>
					<label className={'cursor-pointer flex items-center gap-1.5'} htmlFor="$10K">
						<input {...props} type={'radio'} id={'$10K'} value={'$10K+'} name={'budget'} />
