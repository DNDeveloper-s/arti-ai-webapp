...
export default function useConversations() {
	...
	const isConversationLoading = (state.loading.conversations && conversations?.length === 0) && (!state.conversation?.list || state.conversation?.list?.length === 0);

    const getConversationById = useCallback((conversationId: string) => {
        return state.conversation.map[conversationId];
    }, [state.conversation])

    return {state, conversations, isConversationLoading, getConversationById};
}