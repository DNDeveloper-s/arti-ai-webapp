...
export function timeSince(date: number | Date | string) {
  ...
  if (interval > 1) {
    return (
      Math.floor(interval) + " min" + (Math.floor(interval) > 1 ? "s" : "")
    );
  }
...
export default function getJSONObjectFromAString(inputString: string) {
  ...
  while (startIndex < inputString.length) {
    ...
    if (startBracketIndex !== -1 && endBracketIndex !== -1) {
      let jsonObject = inputString.substring(
        startBracketIndex,
        endBracketIndex + 1
      );
      ...
      while (jsonObject.indexOf("{", 1) !== -1) {
        ...
        if (nestedEndIndex !== -1) {
          jsonObject += inputString.substring(
            endBracketIndex + 1,
            nestedEndIndex + 1
          );
...
export function isValidJsonWithAdsArray(
  inputString: string,
  isIndentedString?: boolean
): boolean {
  try {
