...
export async function POST(req: Request) {
	try {
		...
		if(!formValues) return NextResponse.json({status: 400, message: 'values is required in the request body.'})
		const mailOptions = {
		...
		return NextResponse.json({ok: true, message: 'We will get back to you soon.'})
	} catch(e: unknown) {
