...
export default function Email() {
    ...
    const handleCreateTemplate = async () => {
        const { templateId, createTemplateError } = await createTemplate(templateName, mailContent);
        if (templateId) {
            setSnackBarData({ status: 'success', message: 'Template has been created!' });
    ...
    return <div className="h-full p-10">
        <div className="flex flex-col items-center">
            Email Template
            <input onChange={handleTemplateNameChange} className="text-black rounded-lg p-2 mt-5" placeholder='Template Name' />
            <div className='flex mt-4'>
                <textarea defaultValue={mailContent} onChange={handleChange} className="text-black rounded-lg p-5 mr-5 h-[300px] w-[300px]"></textarea>
            ...
            <button onClick={handleCreateTemplate} className="p-4 bg-green-700 mt-4 rounded-md mb-4">
                Create Template
            </button>
            {selectedTemplateId ? selectedTemplateId : <></>}
            {selectedTemplateId ? <Link href={`/email/${selectedTemplateId}`} className="p-4 bg-green-700 mt-4 rounded-md mb-4" >Create Campaign</Link> : <></>}
        </div>
