...
const WhyUsCard: React.FC<WhyUsCardProps> = ({item, groupId, expand, handleExpand}) => {
	...
	return (
		<div className="arti-card cursor-pointer" ref={artiCardRef} style={{'--mouse-x': `${mousePos.x}px`, '--mouse-y': `${mousePos.y}px`}}>
			<div className="p-6 relative h-full bg-gray-950 rounded-[inherit] z-20 overflow-hidden  font-diatype" onClick={toggleExpand}>
				<div className="z-50">
					...
					<PiCaretRightBold style={{transform: `rotate(${expand ? -90 : 0}deg)`}} className="md:hidden transition-all absolute top-5 right-5" />
					{item.icon && (
					...
					<p className="relative text-sm leading-5 z-10 text-gray-400">{item.overview}</p>
