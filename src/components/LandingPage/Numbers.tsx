...
const NumberCard = ({card} : {card: NumberCard}) => {
	...
	return (
		<div ref={containerRef} className="arti-card h-44 rounded" style={{'--mouse-x': `${mousePos.x}px`, '--mouse-y': `${mousePos.y}px`}}>
			<div className="p-3 md flex gap-2 items-center justify-center relative h-full bg-gray-950 w-full rounded-[inherit] z-20 overflow-hidden scale-100 font-diatype group">
