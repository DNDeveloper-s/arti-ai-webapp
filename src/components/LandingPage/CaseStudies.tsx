...
function CaseStudyContent({ item }: { item: CaseStudyItem }) {
  return (
    <div
      className={
        "h-full flex flex-col justify-center md:justify-center divide-y divide-gray-800 p-4"
      }
    >
      <Element content={item.brief.client_name} type="div" className="pb-4">
        <Element
          content={item.brief.client_name}
          type={"h3"}
          className={"text-3xl font-extrabold pb-1"}
        />
        <Element
          content={item.brief.client_sector}
          type={"p"}
          className={"text-base text-primary"}
        />
      </Element>
      <Element
        content={item.brief.oneLiner}
        type={"p"}
        className="pt-3 md:pt-9 text-sm leading-relaxed text-gray-400"
      />
      <CTAButton
        className="py-2 px-4 text-sm rounded mt-4 md:mt-12"
  ...
  [CaseStudy.MIDTOWN_EAST]: [
    {
      id: "1",
      el: (
        <div key={"1"} className={"w-[240px] flex-shrink-0"}>
    ...
    },
    {
      id: "2",
      el: (
        <div key={"2"} className={"w-[240px] flex-shrink-0"}>
    ...
    },
    {
      id: "3",
      el: (
        <div key={"3"} className={"w-[240px] flex-shrink-0"}>
...
export function AnimatedAdPreview({
  isActive,
  previews = [],
}: {
  isActive: boolean;
  ...
  return (
    <Element
      content={preview.id}
      type={"div"}
      className="w-full flex items-center justify-center gap-7"
    >
...
function CaseStudyImage({ item }: { item: CaseStudyItem }) {
  return (
    <Element
      content={item.images.preview_image}
      type={"div"}
      className="w-full flex items-center justify-center gap-7"
    >
...
const CaseStudies: FC<CaseStudiesProps> = (props) => {
  ...
  return (
    <div
      ...
      className="px-3 py-12 md:py-16 pt-16 md:pt-20"
    >
      <Element
        content={caseStudiesData.headLine}
        type={"h2"}
        className={"text-center mb-5 md:mb-0"}
      >
        <span className={"landing-page-grad-title"}>
      ...
      <div className="flex-col-reverse pl-0 gap-0 md:gap-1 flex md:flex-row py-10 md:py-20 pt-0 md:pt-0 justify-center">
        <>
          <div
            className={
              "flex flex-col relative justify-between w-full md:w-[40%] md:max-w-[520px] px-6"
            }
          >
            <div
              className={
                "absolute top-1/2 -left-2 transform -translate-y-1/2 flex items-center gap-2 group cursor-pointer"
              }
            ...
            >
              {caseStudiesData.items.map((item) => (
                <CaseStudyContent item={item} key={item.id} />
              ))}
            </SwipeableViews>
            <div
              className={
                "absolute top-1/2 -right-2 transform -translate-y-1/2 flex items-center gap-2 group cursor-pointer"
              }
              onClick={() => handleChange(1)}
            >
              <div>
                <RxCaretRight
                  className={
                    "text-5xl transition-all " +
                    (activeTab === caseStudiesData.items.length - 1
                      ? "text-gray-700"
                      : "")
                  }
                />
              ...
              {!isSmallScreen
                ? caseStudiesData.items.map((item) => (
                    <div
                      key={item.id}
                      className={
                        "h-full flex gap-4 p-3 justify-center items-center"
                      }
                    >
