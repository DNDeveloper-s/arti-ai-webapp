'use client';
import React, { FC, FormEvent, HTMLInputTypeAttribute, useContext, useState } from 'react';
...
const AuthForm: FC<AuthFormProps> = (props) => {
	...
	const [formValues, setFormValues] = useState<AuthFormProps['initValues']>(props.initValues);
	const [isSubmitting, setIsSubmitting] = useState<boolean>(false);
	const { formFields } = props;

	const isRequiredValid = (val: string) => val !== null && val !== undefined && val.trim().length !== 0;

	const isFormValid = () => {
		return formFields.every(field => {
			if (field instanceof Array) {
				return field.some(_field => !_field.not_required && isRequiredValid(formValues[_field.name] ?? ''))
			}
			return field.not_required || isRequiredValid(formValues[field.name] ?? '')
		})
	}
	function reset() {
		...
		<div className="w-screen h-screen flex items-center justify-center">
			<div className="w-screen max-w-[300px]">
				<div className="flex flex-col items-center justify-center">
					...
					<h1 className="text-xl font-diatype mt-3 mb-4">{props.formHeading}</h1>
				</div>
				<form onSubmit={handleRegister} action="">
					{formFields.map(formField => {
						if (formField instanceof Array) {
							return <div key={formField[0]?.id ?? 'new-id'} className="flex gap-4">
								{formField.map(formField => (
									<div key={formField.name} className="mb-3">
										<label className="text-sm text-secondaryText" htmlFor="">{formField.label}
											{!formField.not_required && <span className="text-red-600">*</span>}
										</label>
										<input required={!formField.not_required} type={formField?.type ?? 'text'} value={formValues[formField.name]} onChange={(e) => onInputChange(formField.name, e.target.value)}
											className={'w-full mt-1 bg-secondaryText bg-opacity-25 outline-none border-2 border-opacity-0 border-red-600 rounded-lg text-md py-2 px-3 transition-all ' + (false ? 'border-opacity-100' : '')} />
									</div>
								))}
							</div>
						}
						return (
							<div key={formField.name} className="mb-3">
								...
								<input required={!formField.not_required} type={formField?.type ?? 'text'} value={formValues[formField.name]} onChange={(e) => onInputChange(formField.name, e.target.value)} className={'w-full mt-1 bg-secondaryText bg-opacity-25 outline-none border-2 border-opacity-0 border-red-600 rounded-lg text-md py-2 px-3 transition-all ' + (false ? 'border-opacity-100' : '')} />
							</div>
						)
					})}
					<button disabled={!isFormValid()} type='submit' className="disabled:opacity-10 w-full mt-6 flex items-center h-12 justify-center bg-primary outline-none border-2 border-opacity-0 border-red-600 rounded-lg text-md py-2 px-3 transition-all">
