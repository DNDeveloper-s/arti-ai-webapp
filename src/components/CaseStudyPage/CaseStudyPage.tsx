...
const CaseStudyItem: FC<CaseStudyItemProps> = ({item}) => {
	return (
		<div>
			<Element content={item.clientDetails.name} type={'h3'} className="text-xl font-medium mt-3" />
			<Element content={item.oneLiner} type={'h2'} className="text-3xl font-medium text-primary mt-2" />
			<div>
				<h4 className="text-xl font-medium mt-5">Background and Challenge</h4>
				<p className="text-gray-400 text-sm mt-1 leading-6">Midtown East Physical Therapy, a New York-based clinic, excels in offering comprehensive physical therapy services. However, they faced a significant challenge in digital marketing. Their social media campaigns, consisting of lackluster image ads, failed to capture audience attention and generate leads, hindering their online growth.</p>
			</div>
			<div>
				<h4 className="text-xl font-medium mt-5">Objective</h4>
				<p className="text-gray-400 text-base mt-1 leading-6">Our goal was to transform their digital presence, leveraging advanced marketing technologies to drive engagement and patient acquisition.</p>
			</div>
			<div>
				<h4 className="text-xl font-medium mt-5">Approach and Solution</h4>
				<p className="text-gray-400 text-base mt-1 leading-6">Our team initiated a deep dive into the clinic's marketing strategy, employing AI-driven tools for a comprehensive analysis. This included competitor benchmarking and market trends evaluation to identify key areas of improvement. We then harnessed generative AI to design and execute a series of captivating, targeted ads (image and text), each crafted to resonate with the clinic's audience. Our approach was not only creative but also data-driven, ensuring each ad was optimized for maximum impact. Below are some of our best performing ADs:</p>
				<div className="flex justify-center gap-6 mt-3">
					<div className="h-64">
						<Image className={'h-full w-auto'} src={carouselImage1} alt={"Arti AI"} />
					</div>
					<div className="h-64">
						<Image className={'h-full w-auto'} src={carouselImage2} alt={"Arti AI"} />
					</div>
				</div>
			</div>
			<div>
				<h4 className="text-xl font-medium mt-5">Outcomes</h4>
				<div className="text-gray-400 text-base mt-1 leading-6">
