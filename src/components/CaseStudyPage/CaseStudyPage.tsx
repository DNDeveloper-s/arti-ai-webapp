...
import {
  AnimatedAdPreview,
  previewsObj,
} from "@/components/LandingPage/CaseStudies";
...
function OutcomeItem({ label, number, decrease }: OutcomeItemProps) {
  return (
    <div
      className={"grid grid-cols-1 md:grid-cols-[130px_1fr] gap-4 md:gap-10"}
    >
      <div className="md:whitespace-nowrap flex flex-col px-1 md:px-1">
        <div>
          <span className="text-6xl md:text-7xl mb-4 block text-primary font-giasyr">
            ...
            />
            <span className="text-3xl md:text-4xl ml-1 text-primary font-giasyr">
              {number.postString}
            </span>
          </span>
          <p
            className={
              "text-sm md:text-base font-light block text-left text-gray-400"
            }
          >
            {label}
          </p>
      ...
      <div
        className={
          "flex flex-row md:flex-col gap-2 items-start justify-start md:justify-end"
        }
      >
...
const CaseStudyItem: FC<CaseStudyItemProps> = ({ item }) => {
  ...
  return (
    <div>
      <div
        className="relative w-full py-10 md:py-20 flex items-center max-h-[70vh]"
      ...
      >
        <div
          ...
          style={{
            ...
            backgroundImage:
              "url(/assets/images/landing-page/case_study_bg.svg)",
          }}
        />
        <div className="landing-page-section z-10 bg-transparent relative">
          <Element
            content={item.clientDetails.name}
            type={"h3"}
            className="text-xl md:text-2xl font-medium mt-3 text-gray-400"
          />
          <Element
            content={item.oneLiner}
            type={"h2"}
            className="font-giasyr text-4xl md:text-7xl font-medium !leading-snug text-primary mt-10"
          />
        </div>
      </div>
      <CardAnimation items={previewsObj["ant-streetwear"]} autoPlay={true} />
      <Element
        content={item.images?.main_image && item.images?.main_image.length > 0}
        type={"div"}
          ...
          <div className="flex justify-center gap-6 my-10">
            {item.images?.main_image?.map((image, index) => (
              <>
                <div
                  style={{
                    width: `calc(100% / ${item.images?.main_image?.length})`,
                  }}
                >
                  <Image
      ...
      <Element
        content={item.service_info}
        type={"div"}
        className={"landing-page-section flex flex-col gap-14"}
      >
        {item.service_info?.map((service, index) => (
          <div
            className="flex flex-col gap-5 md:gap-6 md:flex-row items-start"
            key={index}
          >
            ...
            <div className="flex flex-wrap gap-4 md:gap-6">
              {service.items.map((item, index) => (
                <div
                  className="py-1 px-3 text-base rounded-full border border-white"
                  key={index}
                >
                  <span>{item}</span>
                </div>
      ...
      >
        {item.sections
          .sort((a, b) => (a.serialOrder ?? 0) - (b.serialOrder ?? 0))
          ?.map((section, index) => (
            <div
              className={
                "grid grid-cols-1 md:grid-cols-2 gap-14 md:gap-5 mt-14 md:mt-20"
              }
              key={index}
            >
              {index % 2 !== 0 && (
                <div className={section.sideContent ? "" : "hidden md:block"}>
                  {section.sideContent ?? null}
                </div>
              )}
              <div>
                ...
                <p className="text-gray-400 text-base md:text-lg mt-6 md:mt-8 leading-8">
                  {section.description}
                </p>
              </div>
              {index % 2 === 0 && (
      ...
      <CardAnimation
        ContainerClassName="h-[450px] mt-20"
        items={previewsObj[item.id]}
        autoPlay={true}
      />
      <Element
        content={item.outcomes}
        type={"div"}
        className={"landing-page-section"}
      >
      ...
      <Element
        content={item.clientQuote?.description}
        type={"div"}
        className={"landing-page-section"}
      >
        <div className={"mt-8 md:mt-16 py-10 md:py-24"}>
          ...
          <div
            className={
              "text-3xl md:text-5xl !leading-snug text-white font-giasyr"
            }
          >
            {item.clientQuote?.description}
          </div>
          {item.clientQuote?.author && (
      ...
      <Element
        content={item.lowerSections}
        type={"div"}
        className={"landing-page-section"}
      >
        <div
          className={"grid grid-cols-1 md:grid-cols-2 mt-14 md:mt-20 gap-10"}
        >
          {item.lowerSections
            ?.sort((a, b) => (a.serialOrder ?? 0) - (b.serialOrder ?? 0))
            ?.map((section, index) => (
              <div key={section.id}>
                ...
                <p className="text-gray-400 text-base md:text-lg mt-6 md:mt-8 mb-3 leading-8">
                  {section.description}
                </p>
                {section.containsCta && (
                  <CTAButton>
