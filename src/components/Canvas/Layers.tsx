...
const LayerItem: FC<LayerItemProps> = ({
  ...
  useEffect(() => {
    console.log(
      "testing context menu of layers | show, position - ",
      show,
      position
    );
  }, [show, position]);
  ...
  return (
    <>
      <div
        ref={containerRef}
        className={
          "h-8 w-full p-2 flex items-center relative text-sm gap-2 text-white " +
          (selected ? " bg-gray-600" : "")
        }
        onClick={() => props.handleSelectLayer(props.item.id)}
      >
        ...
        {!editMode ? (
          <span
            onClick={(e) => {
              e.detail === 2 && setEditMode(true);
            }}
            className={"block select-none flex-1 truncate px-1"}
          >
            {props.item.name}
          </span>
        ) : (
          <div className={"flex-1 relative"}>
            <input
              ...
              type="text"
              className={
                "rounded-[1px] px-1 w-full focus:outline-blue-500 focus-within:outline-none border-none bg-transparent text-white"
              }
              value={editValue}
      ...
      <Portal id="contextmenuportal">
        {show ? (
          <div
            ref={contextMenuRef}
            className="fixed min-w-[100px] bg-gray-800 shadow-lg text-white overflow-hidden rounded"
            style={{ top: position.y, left: position.x }}
          >
            <div
              onClick={handleDeleteClick}
              className="flex items-center select-none cursor-pointer gap-1.5 py-1.5 px-3 hover:bg-gray-900 active:bg-gray-950"
            >
...
const Layers: FC<LayersProps> = ({
  ...
  function onListChange(newList: Element[]) {
    // setListItem(newList);
    const sortedList = newList.map((i, ind) => ({
      ...i,
      order: newList.length - 1 - ind,
    }));
  ...
  const handleUpdateName = useCallback(
    (itemId: any, name: string) => {
      setElements((prevElements: any) => {
        const elIndex = prevElements.findIndex(
          (el: Element) => el.id === itemId
        );
  ...
  return (
    <div
      className={
        "w-[200px] bg-gray-900 rounded py-3 flex flex-col overflow-hidden max-h-[300px]"
      }
    >
      <div
        className={"px-3 pb-2 flex gap-2 items-center text-sm text-gray-200"}
      >
        <IoLayers />
        <span className={"font-medium text-gray-300"}>Layers</span>
      </div>
      {disabled ? (
        <div className="flex-1 py-1 px-2 text-xs text-gray-500 justify-center items-center flex">
          ...
          <DraggableList
            ...
            template={(props) => (
              <LayerItem
                key={props.item.id}
                itemActions={itemActions}
                selected={props.item.id === selectedElement}
