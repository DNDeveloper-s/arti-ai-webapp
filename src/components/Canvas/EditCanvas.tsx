...
export function parseRoughState(
  element: Pick<Element, "x1" | "x2" | "y1" | "y2" | "type">,
  roughOptions: Options
) {
...
function createElement(
  ...
  switch (elementType) {
    ...
    case "text":
      return {
        id,
        name: elDetails?.name ?? elementType,
        order: elDetails?.order ?? id,
        type: elementType,
        x1,
        y1,
        x2,
...
function adjustElementCoordinates(
  element: Pick<Element, "x1" | "y1" | "x2" | "y2" | "type">
) {
...
function lineSegmentIntersectsRectangle(segment: Point[], rectangle: any) {
  ...
  if (
    (x1 > rx &&
      x1 < rx + width &&
      ((y1 > ry && y1 < ry + height) || (y2 > ry && y2 < ry + height))) ||
    (x2 > rx &&
      x2 < rx + width &&
      ((y1 > ry && y1 < ry + height) || (y2 > ry && y2 < ry + height)))
  ) {
...
function isElementIntersecting(rect: Coords, element: Element) {
  if (element.type === "line") {
    ...
    return isOverlappingRectangle(rect, {
      x1: element.x1,
      y1: element.y1,
      x2: element.x2,
      y2: element.y2,
...
function pointNearLine(
  ...
  // Calculate distances between the point and the endpoints
  var distanceToP1 = distance(x, y, x1, y1);
  var distanceToP2 = distance(x, y, x2, y2);

  // Calculate distance between the endpoints of the line segment
  var lineLength = distance(x1, y1, x2, y2);

  // Check if the point is close to the line segment within the threshold
  return distanceToP1 + distanceToP2 <= lineLength + threshold;
}

function isWithinElement(
  x: number,
  y: number,
  element: Pick<Element, "type" | "x1" | "y1" | "x2" | "y2">
) {
  let is = false;
  switch (element.type) {
    case "text":
    ...
    case "ellipse":
      ...
      is =
        Math.abs(y - y1e) <= Math.abs(y2e - y1e) &&
        Math.abs(x - x1e) <= Math.abs(x2e - x1e);
      break;
    default:
      break;
  }
...
function getElementAtPosition(
  ...
  const arr = elements.reverse().map((element) => ({
    ...element,
...
function legacy_positionWithinElement(
  x: number,
  y: number,
  element: Pick<Element, "type" | "x1" | "y1" | "x2" | "y2">
): Position {
...
function positionWithinElement(
  ...
  const { type, x1, x2, y1, y2 } = element;
  if (type === "rectangle" || type === "image") {
    const rightMiddle = isWithinElement(x, y, {
      type: "line",
      x1: x2,
      y1: y1,
      x2: x2,
    ...
    const leftMiddle = isWithinElement(x, y, {
      type: "line",
      x1: x1,
      y1: y1,
      x2: x1,
    ...
    const bottomMiddle = isWithinElement(x, y, {
      type: "line",
      x1: x1,
      y1: y2,
      x2: x2,
    ...
    const topMiddle = isWithinElement(x, y, {
      type: "line",
      x1: x1,
      y1: y1,
      x2: x2,
...
function positionWithinSelectedRect(
  x: number,
  y: number,
  rectElements: SelectedRect[]
) {
  const arr = rectElements.reverse().map((element) => ({
    ...element,
    position: positionWithinElement(x, y, {
      ...element.coords,
      type: "rectangle",
    }),
  }));
...
function positionWithinSelectionElement(
  x: number,
  y: number,
  ...
  if (element.type === "rectangle") {
    ...
    const rightMiddle = isWithinRectangle(
      x,
      y,
      rm.x1,
      rm.y1,
    ...
    const bottomRight = isWithinRectangle(
      x,
      y,
      br.x1,
      br.y1,
    ...
    const bottomMiddle = isWithinRectangle(
      x,
      y,
      bm.x1,
      bm.y1,
    ...
    const bottomLeft = isWithinRectangle(
      x,
      y,
      bl.x1,
      bl.y1,
    ...
    const leftMiddle = isWithinRectangle(
      x,
      y,
      lm.x1,
      lm.y1,
    ...
    const rotateHandle = isWithinRectangle(
      x,
      y,
      element.coords.handle.x1,
      element.coords.handle.y1,
      element.coords.handle.x2,
      element.coords.handle.y2,
      "rotate"
    );
    ...
    const startCorner = isWithinRectangle(
      x,
      y,
      start.x1,
      start.y1,
    ...
    const endCorner = isWithinRectangle(
      x,
      y,
      end.x1,
      end.y1,
...
function getProportionalCoords(
  x: number,
  y: number,
  x1: number,
  y1: number,
...
function legacy_resizedCoordinates(
  x: number,
  y: number,
  type: ElementType,
  position: Position,
  element: Coords,
  isSelectionRect?: boolean
) {
  const { x1, y1, x2, y2 } = element;
...
const getSelectionElementCoords = (
  type: "line" | "rectangle",
  clientX: number,
  clientY: number,
  _x1: number,
  ...
  const position = positionWithinSelectionElement(
    clientX,
    clientY,
    selectionEl as SelectionElement
  );
...
const useHistory = (initialState: any) => {
  ...
  const setState = useCallback(
    (action: any, overwrite = false, updateLastIndex = false) => {
      let callParent = 0;
  ...
  switch (type) {
    ...
    case "circle":
      ...
      rect = {
        ...
        type,
        offsetX: x - (x1 - radius - 4),
        offsetY: y - (y1 - radius - 4),
      };
      break;
    case "ellipse":
      const _coords = {
      ...
      rect = {
        coords: _coords,
        type,
        offsetX: x - _coords.x1,
        offsetY: y - _coords.y1,
      };
      break;
    default:
      break;
...
function getElementCoordinatedWithElementRect(
  rectCoords: Coords,
  type: ElementType,
  element: Element
) {
  switch (type) {
    case "line":
      return {
        x1: rectCoords.x1 + 4,
        y1: element.y1 > element.y2 ? rectCoords.y1 - 4 : rectCoords.y1 + 4,
        x2: rectCoords.x2 - 4,
        y2: element.y1 > element.y2 ? rectCoords.y2 + 4 : rectCoords.y2 - 4,
      };
    case "text":
    case "rectangle":
    case "image":
    ...
    case "circle":
      ...
      return {
        x1: rectCoords.x1 + radius + 4,
        y1: rectCoords.y1 + radius + 4,
        x2: rectCoords.x2 - 4,
        y2: (rectCoords.y2 - 4 + (rectCoords.y1 - 4)) / 2,
      };
    case "ellipse":
...
export default function EditCanvas({
        ...
        (elements: any) => {
          ...
          if (type === "text") {
            ...
            const createdElement = createElement(
              ...
              y1,
              x1 + (textWidth ?? 0),
              lines[lines.length - 1][2] + +fontSize,
              null,
              elDetails
            );
            newElements[index] = {
              ...
              customOptions: {
                ...
                fillStyle:
                  options.fillStyle ??
                  newElements[index].customOptions?.fillStyle ??
                  "rgba(0,0,0,1)",
                fontSize,
            ...
            newElements[index] = createElement(
              ...
              x2,
              y2,
              Object.keys(options ?? {}).includes("text") || !options
                ? newElements[index].roughOptions
                : { ...newElements[index].roughOptions, ...(options ?? {}) },
              elDetails
            );
          }
      ...
      setTimeout(() => {
        if (textAreaRef.current) {
          ...
          textAreaRef.current.value =
            selectedElement?.customOptions?.text ?? "";
        }
      }, 10);
    }
  ...
  function handleMouseDown(e: any) {
    ...
    if (elementType === "selection") {
      const selectionElement = getElementAtPosition(
        clientX,
        clientY,
        selectionElements
      );
      const element = getElementAtPosition(clientX, clientY, elements);
      const targetRectElement = positionWithinSelectedRect(
        clientX,
        clientY,
        elementRects
      );
      if (selectionElement) {
        ...
        setSelectionElements((selectionElements) => [
          {
            ...selectionElement,
            offsetX: clientX1 - selectionElement.x1,
            offsetY: clientY1 - selectionElement.y1,
          },
        ]);
        setSelectedElements((prevElements) =>
          prevElements.map((el) => {
            ...
            return {
              ...element,
              offsetX: clientX1 - element.x1,
              offsetY: clientY1 - element.y1,
            };
          })
        );
        // console.log('setting element rects - ');
        setElementRects((prevRects) =>
          prevRects.map(({ coords, type }) => {
            return {
              coords,
              type,
              offsetX: clientX1 - coords.x1,
              offsetY: clientY1 - coords.y1,
            };
          })
        );
      } else if (element) {
        ...
        // }
        targetRectElement?.position
          ? setAction(
              targetRectElement?.position === "inside"
                ? Action.MOVE
                : Action.RESIZE
            )
          : setAction(Action.MOVE);
    ...
    function handleMouseMove(e: any) {
      ...
      if (elementType === "selection") {
        ...
        if (selectionElement) {
          ...
          const element = positionWithinSelectedRect(
            clientX,
            clientY,
            elementRects
          );
      ...
      const selectionElementSelected = selectionElements[
        selectionElements.length - 1
      ]
        ? isWithinElement(
            clientX,
            clientY,
            selectionElements[selectionElements.length - 1]
          )
        : false;
      if (action === Action.DRAW && selectedElement) {
        ...
        if (elementRects && elementRects.length > 0) {
          const newRects = [...elementRects]
            .map(({ coords: element, offsetX, offsetY, type }) => {
              const x1 = clientX - (offsetX ?? 0);
              const y1 = clientY - (offsetY ?? 0);
              const x2 = x1 + (element.x2 - element.x1);
              const y2 = y1 + (element.y2 - element.y1);
        ...
        if (selectionElementSelected) {
          const element = selectionElements[
            selectionElements.length - 1
          ] as SelectedElement;
          if (element) {
            const { offsetX, offsetY } = element;
            ...
            newElements[newElements.length - 1] = {
              ...element,
              ...createElement(element.id, "rectangle", x1, y1, x2, y2, {
                strokeLineDash: [3, 5],
              }),
            };
          ...
          for (let i = 0; i < selectedElements.length; i++) {
            ...
            updateElement(element.id, x1, y1, x2, y2, element.type, {
              ...
              text: element.customOptions?.text,
              font: element.customOptions?.font,
            });
          }
        ...
        const { id, type, position, customOptions, ...coordinates } =
          selectedElement;
        if (position) {
          ...
          if (elementRects.length === 1) {
            ...
            const rect = getElementCoordinatedWithElementRect(
              { x1, y1, x2, y2 },
              type,
              selectedElement
            );
            updateElement(id, rect.x1, rect.y1, rect.x2, rect.y2, type, {
              ...customOptions,
        ...
        newElements[newElements.length - 1] = createElement(
          element.id,
          "rectangle",
          element.x1,
          element.y1,
        ...
        const intersectedElements = elements.filter((_element: any) =>
          isElementIntersecting({ x1, y1, x2, y2 }, _element)
        );

        const rects = intersectedElements
          .filter(({ x1, x2, y1, y2 }) => x1 !== x2 || y1 !== y2)
          .map((selectedElement: Element) => {
  ...
  useEffect(() => {
    function handleMouseUp(e: any) {
      ...
      if (selectedElement) {
        if (selectedElement.type === "text" && e.detail === 2) {
          setAction(Action.WRITE);
          return;
        }
      }
      if (
        (action === Action.DRAW || action === Action.RESIZE) &&
        selectedElement
      ) {
  ...
  useLayoutEffect(() => {
    ...
    selectionElements.forEach(({ type, x1, y1, x2, y2, roughOptions, id }) => {
      if (roughOptions) {
        const parsedRough = parseRoughState(
          { x1, y1, x2, y2, type },
          { ...(roughOptions ?? {}), fill: "transparent" }
        );
  ...
  useLayoutEffect(() => {
    ...
    elements
      ...
      .forEach((element: Element) => {
        if (action === Action.WRITE && selectedElement?.id === element.id)
          return;
  ...
  function handleFormatChange(props: any, completed?: boolean) {
    ...
    if (selectedElement) {
      ...
      if (props.color) {
        ...
        if (element?.type === "text") {
          const options = {
            width: element.customOptions?.width,
            italic: props.italic,
            bold: props.bold,
            text: element.customOptions?.text ?? "",
            fontSize: props.fontSize,
            fontFamily: props.fontFamily,
            font: `${props.italic ? "italic " : ""}${props.bold ? "bold " : ""}${props.fontSize}px ${props.fontFamily}`,
            fillStyle: element.customOptions?.fillStyle,
          };
          element &&
            updateElement(
              element.id,
  ...
  async function handleImageChange(e: any) {
    ...
    img.onload = function () {
      ...
      const element = createElement(id, "image", 100, 100, width, height, {
        src: img.src,
      });
  ...
  function exportCanvas() {
    ...
    if (bgImage) {
        ...
        try {
          ...
          canvasEl.toBlob((blob) => {
            ...
            handleExport(
              ...
              undefined,
              bgImage === imageUrl ? undefined : bgImage
            );
          });
          ...
          setExporting(false);
          setSnackbarData({
            message: "Error exporting the image. Please try again later!",
            status: "error",
          });
        }
      };
      ...
      img.src = bgImage.startsWith("blob:")
        ? bgImage
    ...
    <>
        ...
        {action === Action.WRITE && selectedElement && (
          <textarea
            ...
            style={{
              ...
              width: elementRects[0]
                ? elementRects[0].coords.x2 - elementRects[0].coords.x1 + 10
                : 100,
              height: elementRects[0]
                ? elementRects[0].coords.y2 - elementRects[0].coords.y1
                : 50,
              resize: "none",
        ...
        {action !== Action.WRITE &&
          ...
          elementRects.length === 1 && (
            ...
            >
              ...
              {["rectangle", "ellipse", "line", "circle"].includes(
                selectedElement.type
              ) && (
                <ShapeTools
                  initialShapeDetails={selectedElement?.roughState?.options}
                  handleFormatChange={handleFormatChange}
                />
              )}
            </div>
      ...
      <Portal id="canvastoolsportal">
        <div
          ref={toolItemRef}
          style={{
            opacity: containerPos.top !== 0 && containerPos.left !== 0 ? 1 : 0,
            top: containerPos.top,
            left: containerPos.left,
          }}
          className="absolute left-[calc(100%+10px)] top-1/2 transform -translate-y-1/2 z-20"
        >
          <EditTools
            initialFontDetails={
              selectedElement?.customOptions
                ? (selectedElement?.customOptions as FontFormat)
                : undefined
            }
      ...
      <div
        className="flex justify-end px-4 items-center gap-3 mt-3"
        onClick={() => {}}
      >
        <button
          onClick={exportCanvas}
          className="bg-primary text-white text-xs px-4 py-1.5 leading-[16px] rounded cursor-pointer"
        >
          Done
        </button>
