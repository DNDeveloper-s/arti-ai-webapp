...
import { VariantsByConversation } from "@/api/conversation";

export const AdCreativeCardShimmer = () => {
  return (
    <div
      className={
        "animate-pulse w-[25rem] flex-shrink-0 pb-4  relative border-2 border-secondaryBackground transition-all rounded-xl overflow-hidden text-[9px] bg-secondaryBackground"
      }
    >
      ...
      <div className="py-3 px-3 relative flex items-center justify-between z-20">
        <h2 className="app-shimmer whitespace-nowrap w-full overflow-hidden overflow-ellipsis mr-5 text-base font-medium text-primary">
          This is a normal object where you can have some other things to adjust
        </h2>
        <span className="whitespace-nowrap">
          <span className="text-white text-opacity-0 text-[1.1em]">
            Generated:
          </span>
          <span className="app-shimmer text-primary text-[1.1em]">
            2 sec ago
          </span>
        </span>
      </div>
      <div className={"flex items-start gap-3 px-3"}>
        ...
        {/*))}*/}
        <FacebookAdVariantShimmer
          style={{ zoom: 0.3, fontSize: "9px" }}
          className="flex-shrink-0 p-[3.8em] border border-gray-800 bg-secondaryBackground rounded max-w-[30%]"
        />
        <FacebookAdVariantShimmer
          style={{ zoom: 0.3, fontSize: "9px" }}
          className="flex-shrink-0 p-[3.8em] border border-gray-800 bg-secondaryBackground rounded max-w-[30%]"
      ...
      <div className={"flex items-start gap-3 px-3"}>
        {variants
          ...
          .map((currentAdVariant) => (
            <FacebookAdVariantMini
              style={{ zoom: 0.3, fontSize: "9px" }}
              key={currentAdVariant.id}
              noExpand={true}
              adVariant={currentAdVariant}
              className="flex-shrink-0 p-[3.8em] border border-gray-800 bg-secondaryBackground rounded max-w-[30%]"
            />
          ))}
      </div>
