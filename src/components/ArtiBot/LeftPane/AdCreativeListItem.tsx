...
const AdCreativeListItem: FC<AdCreativeListItemProps> = ({conversationId}) => {
    ...
    return (
      <Link href={getConversationURL(conversationId, ConversationType.AD_CREATIVE) + '?ad_creative=expand'} key={conversationId} className={'flex gap-4 items-start px-4 py-3 text-gray-300 cursor-pointer overflow-hidden transition-all text-sm leading-6'}>
