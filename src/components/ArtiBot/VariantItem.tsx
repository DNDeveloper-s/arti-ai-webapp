...
const VariantItem: FC<VariantTabProps> = ({
  mock = new Mock(),
  width,
  activeVariant,
  ...props
  ...
  const editMode =
    editState.variant && editState.variant.id === activeVariant.id;
  const [, setSnackbarData] = useContext(SnackbarContext).snackBarData;
  const [updating, setUpdating] = useState(false);
  const [downloading, setDownloading] = useState(false);

  const variant = mock.is
    ? activeVariant
    : (state.variant.map[activeVariant.id] as IAdVariant) ?? null;
        ...
        <>
          ...
          <div ref={variantRef} className={"relative mt-2 w-[80%]"}>
            {!editMode ? (
              <FacebookAdVariant
                adVariant={variant}
                className="p-3 !max-w-unset border !border-gray-800 h-full bg-secondaryBackground rounded-lg"
                style={{
                  fontSize: "8px",
              ...
              <EditFacebookAdVariant
                ...
                className="p-3 !max-w-unset border border-gray-800 h-auto rounded-lg"
                style={{ fontSize: "8px" }}
              />
            )}
            {updating && (
          ...
          {!mock.is && !downloading && !updating && !editMode && (
            <div className="w-[80%] mx-auto mt-2 flex items-center justify-center gap-4">
              <button
                onClick={handleEdit}
