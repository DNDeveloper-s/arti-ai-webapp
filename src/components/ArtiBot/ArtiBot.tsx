...
} from "@/components/ArtiBot/ConversationPage";
...
const ArtiBot: FC<ArtiBotProps> = ({
  ...
  useEffect(() => {
    ...
    const currentConversation = dummy.Conversations.find(
  ...
  function handleChunk(
    ...
    const jsonObjectInString = chunk ? getJSONObjectFromAString(chunk) : null;
  ...
  async function handleGetAdNowButton() {
    ...
    const transformedMessages = _messages
      .filter((a) => a.content)
      .map((c) => ({ role: c.role, content: c.content }));
    try {
      const result = await axios.post(
        ROUTES.MESSAGE.GET_AD_JSON,
        {
  ...
  async function handleSubmitMessage(generate_ad?: boolean) {
    if (
      !generate_ad &&
      (exhausted || (!selectedFiles && inputValue.trim().length === 0))
    )
      return;
    const currentConversation = dummy.Conversations.find(
      (c) => c.id === conversation?.id
    ...
    if (response && !response.ok)
      setSnackBarData({
        message: "We are unable to process the request right now!",
        status: "error",
      });
  ...
  const enableMessageInput = miniVersion
    ? !exhausted
    : !isGeneratingAd &&
      !isGenerating &&
      !saveMessageRef.current &&
      messages?.find((m) => m.generating === true) === undefined;
  const showGetAdNowButton =
    enableMessageInput &&
    messages.length >= threshold.getAdNowButtonAfter &&
    conversation?.conversation_type === ConversationType.AD_CREATIVE;

  const animationShouldBePaused = miniVersion
    ? isInputInFocus || isGenerating || isGeneratingAd
    : true;

  return (
    <div
      className={`${borderAnimation ? `border-animation ${animationShouldBePaused ? "paused" : ""}` : ""} flex h-full overflow-hidden`}
    >
      <div
        className={
      ...
      >
          ...
          {!collapsed ? (
            <>
              <MessageContainer
                ...
                showGetAdNowButton={showGetAdNowButton}
              ...
              <div
                ...
                style={{
                  height: selectedFiles
                    ? "220px"
                    : areaHeight > 0
                      ? `calc(4.5rem + ${areaHeight}px)`
                      : "4.5rem",
                }}
              >
                {showGetAdNowButton && (
                ...
                >
                  {
                    selectedFiles ? (
                      <div className="w-full h-[200px] p-3 px-6 flex absolute bottom-0 bg-background rounded-xl overflow-x-auto">
        ...
        {exhausted && (
          <motion.div
            ...
            initial={"hidden"}
            animate={"show"}
            className="flex z-10 px-4 w-full absolute h-full flex-col items-center justify-center bg-background bg-opacity-80"
          >
            {/*<Logo width={60} height={60} fill={colors.primaryText}/>*/}
            <Logo width={60} height={60} />
            <motion.p
              variants={framerItem()}
              className="text-3xl font-medium text-white font-giasyr"
            >
              Arti
            </motion.p>
            <motion.p variants={framerItem(0.5)} className="text-md my-4">
              Discover the Arti Difference
            </motion.p>
            <motion.p
              variants={framerItem()}
              className="text-sm max-w-lg text-yellow-500 text-center"
            >
              You have exhausted the free tier limit. Please register to get the
              full access
            </motion.p>
            <motion.button
              variants={framerItem()}
              className="my-4 cta-button breathing-button-primary"
