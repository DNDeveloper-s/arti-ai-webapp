...
const RightPane: FC<RightPaneProps> = ({
  ...
  const {
    state: { inProcess, variant, inError, ...state },
  ...
  useEffect(() => {
    ...
    intervalIdRef.current = setInterval(() => {
      setActiveVariant(
        adCreative.variants[
          Math.floor(Math.random() * adCreative.variants.length)
        ]
      );
    }, 10000);
  ...
  const content = (
    <div
      className={
        ...
        (mock.is ? " overflow-hidden" : " overflow-y-auto overflow-x-visible")
      }
    >
      {isAdCampaign ? (
        <AdCampaignStepOne />
        ...
        <>
          <div className="px-4 w-full py-1 mt-3 flex justify-between items-center">
            ...
            {docUrl && (
              <motion.a
                title="Arti AI Generated Ad Creative PDF.pdf"
                initial={{ opacity: 0 }}
                download="Arti AI Generated Ad Creative PDF.pdf"
                animate={{ opacity: 1 }}
                href={docUrl}
