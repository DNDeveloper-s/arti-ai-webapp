...
import FacebookAdVariant, {
  FacebookAdVariantProps,
} from "../FacebookAdVariant";
...
const dummyCanvasState3 =
  '[{"id":1709810490095,"name":"Sample Text","order":1709810490095,"type":"text","x1":281.40625,"y1":40.984375,"x2":381.40625,"y2":96.984375,"customOptions":{"text":"Sample Text","font":"26px Chalkboard SE","fillStyle":"rgba(0,0,0,1)","fontSize":26,"width":100,"fontFamily":"Chalkboard SE","bold":false,"italic":false}}]';

const dummyCanvasState4 =
  '[{"id":1709881257149,"name":"circle","order":1709881257149,"type":"circle","x1":197.90625,"y1":337.59375,"x2":237.90625,"y2":355.59375,"roughOptions":{"fillStyle":"solid","fill":"rgba(231, 32, 32, 1)","stroke":"rgba(0, 0, 0, 0)"}},{"id":1709881267315,"name":"rectangle","order":1709881267315,"type":"rectangle","x1":80.90625,"y1":315.59375,"x2":312.90625,"y2":362.59375,"roughOptions":{"fillStyle":"solid","fill":"rgba(231, 32, 32, 1)","stroke":"rgba(0, 0, 0, 0)"}},{"id":1709881329616,"name":"Sample Text","order":1709881329616,"type":"text","x1":132.90625,"y1":328.59375,"x2":276.90625,"y2":352.59375,"customOptions":{"text":"Sample Text","font":"24px Tahoma","fillStyle":"rgba(255, 255, 255, 1)","fontSize":24,"width":144,"fontFamily":"Tahoma","bold":false,"italic":false}}]';

const CanvasImageItem: FC<CanvasImageItemProps> = ({
  isActive,
  item,
  onClick,
}) => {
  return (
    <div
      className={
        "border border-transparent relative aspect-square rounded " +
    ...
    <div className="grid grid-cols-2 gap-3 w-full">
      {props.list.map((item, index) => (
        <CanvasImageItem
          ...
          isActive={props.activeItem === index}
        />
      ))}
      {[1, 2].slice(props.list.length).map((c) => (
        <div key={c} className="aspect-square rounded bg-black"></div>
      ))}
    </div>
  );
};
...
export const EditFacebookAdVariant: FC<EditFacebookAdVariantProps> = ({
  showConfirmModal,
  containerClassName,
  setShowConfirmModal,
  mock = new Mock(),
  ...
  const {
    state: { inError, inProcess, variant },
    dispatch,
  } = useConversation();
  ...
  const imageContainerJSX =
    imageUrl === "error" ? (
      ...
      <Image1
        ...
        height={100}
        className="mb-[0.5em] w-full"
        src={editVariantState?.variant?.imageUrl ?? imageUrl ?? dummyImage}
  ...
  async function handleEdit(cs: CONTROL_STATE, key: REGENERATE_SECTION) {
    ...
    if (cs === CONTROL_STATE.EDIT && key === REGENERATE_SECTION.IMAGE) {
      const imageIndex = editVariantState?.variantImages?.findIndex(
        (c) => c.get("url") === editVariantState.variant?.imageUrl
      );
  ...
  function handleExport(
    ...
    if (!newImage) {
      ...
      if (prevImageObj && _bgImage) {
        ...
        return setSnackbarData({
          message: "Couldn't find the background image!",
          status: "warning",
        });
      }
    }
    ...
    if (newImage) {
      variantImages = [...variantImages, new VariantImageMap(createdImageObj)];
    } else {
  ...
  const imageObject = useMemo(() => {
    return editVariantState?.variantImages
      ? editVariantState?.variantImages[activeTab]
      : undefined;
  }, [editVariantState?.variantImages, activeTab]);

  const imageUrlToEdit =
    imageObject?.get("bgImage") ??
    imageObject?.get("url") ??
    editVariantState?.variant?.imageUrl ??
    imageUrl ??
    adVariant.imageUrl;

  const bgImages: string[] = [
    ...(editVariantState?.variantImages ?? [])
      .map((g) => g.get("bgImage"))
  ...
  return (
    <>
      <Modal
        PaperProps={{ className: "bg-black bg-opacity-90" }}
      ...
      >
        <div className="max-w-[500px]">
          {editVariantState.variant && (
            <FacebookAdVariant
              forceAdVariant={true}
              adVariant={editVariantState.variant}
              className="p-3 !w-full !max-w-unset border border-gray-800 h-full bg-secondaryBackground rounded-lg"
              style={{ fontSize: "10px" }}
            />
          )}
        </div>
      ...
      <Modal
        PaperProps={{ className: "bg-black bg-opacity-90 !min-h-[unset]" }}
      ...
      >
        <div className="max-w-[300px] bg-black text-white p-4 rounded shadow-lg">
          ...
          <p className="my-5 text-gray-300 text-sm">
            Save the Variant changes before leaving?
          </p>
          <div className="w-full flex py-2 px-4 gap-5 items-center justify-end">
            <button
              className={"py-1 px-5 rounded text-base bg-gray-600"}
              onClick={() => setShowConfirmModal(false)}
            >
              <span>Leave</span>
            </button>
            <button
              className={"py-1 px-5 rounded text-base bg-primary"}
              onClick={() => setShowConfirmModal(false)}
            >
              <span>Stay</span>
            </button>
      ...
      <div
        key={adVariant.id}
        className={
          "relative ad-variant text-xs md:text-base !p-0 " + (className ?? "")
        }
        {...props}
      >
        <div
        ...
        <EditControl
          ...
          inputEditable={true}
          content={adVariant.text}
          controlKey={REGENERATE_SECTION.DESCRIPTION}
          containerClassName={
            "z-30 text-[1.6em] leading-[1.6] py-[0.6em] px-[1em] my-2 min-h-[82px] " +
            getBlurClassName(REGENERATE_SECTION.DESCRIPTION)
          }
        ...
        {isEdittingImage ? (
          <>
            <div className="relative w-full aspect-square">
              ...
              >
                ...
                >
                  {(editVariantState?.variantImages ?? []).map(
                    (imageObject, index) => {
              ...
              <EditCanvas
                ...
                canvasState={imageObject?.get("canvasState")}
                handleClose={handleClose}
              ...
              <div className="flex gap-1 flex-col items-start mt-2 py-3">
                <CanvasImageViewer
                  list={[
                    {
                      id: 1,
                      bgUrl: imageObject?.get("bgImage") ?? "",
                      canvasState: dummyCanvasState,
                    },
                    {
                      id: 2,
                      bgUrl: imageObject?.get("bgImage") ?? "",
                      canvasState: dummyCanvasState2,
                    },
                    {
                      id: 3,
                      bgUrl: imageObject?.get("bgImage") ?? "",
                      canvasState: dummyCanvasState3,
                    },
                    {
                      id: 4,
                      bgUrl: imageObject?.get("bgImage") ?? "",
                      canvasState: dummyCanvasState4,
                    },
                  ]}
          ...
          <EditControl
            type={"image"}
            handleClose={handleClose}
            controlKey={REGENERATE_SECTION.IMAGE}
            containerClassName={
              "text-[1.6em] " + getBlurClassName(REGENERATE_SECTION.IMAGE)
            }
            handleEdit={handleEdit}
          >
            {imageContainerJSX}
        ...
        >
          <div className="flex-1">
            <EditControl
              ...
              type={"text"}
              inputEditable={true}
              content={adVariant.oneLiner}
              handleClose={handleClose}
              controlKey={REGENERATE_SECTION.ONE_LINER}
              containerClassName={
                "relative px-2 py-2 text-[1.75em] leading-[1.3em] " +
                getBlurClassName(REGENERATE_SECTION.ONE_LINER)
              }
              handleEdit={handleEdit}
            />
          </div>
      ...
      {!isEdittingImage && !regenerateMap && (
        <div
          className={
            "flex justify-end w-full mt-2 mb-2 items-center gap-1 text-xs"
          }
        >
          <div className={"flex items-center gap-4"}>
            ...
            <div
              ...
              onClick={handleEditVariantClose}
            >
              <span>Close</span>
            </div>
          </div>
