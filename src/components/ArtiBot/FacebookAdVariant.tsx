  ...
  return (
    ...
    >
      ...
      <div className="mb-[1em] px-[1em]">
        <span className="app-shimmer text-[0.95em] inline-flex leading-[1.5em]">
          Than you for providing that information, Saurabh, To better undersant
          yousfka sdlf jalsdf lajflj alsjf lajsdfl jaldf jlasdjf lasdf{" "}
        </span>
      </div>
      ...
      >
        <span className={"app-shimmer text-[1.25em] leading-[1.3em]"}>
          This is a normal one liner where you can show one liner
        </span>
        <div className="flex-shrink-0">
          <span className="app-shimmer cursor-pointer rounded bg-gray-700 px-[0.6em] py-[0.5em] text-[1em]">
            Learn More
          </span>
        </div>
      </div>
...
export interface FacebookAdVariantProps
...
const FacebookAdVariant: FC<FacebookAdVariantProps> = ({
  mock = new Mock(),
  forceAdVariant,
  ...
  const adVariant = forceAdVariant
    ? _adVariant
    : variant.map && variant.map[_adVariant.id]
      ? variant.map[_adVariant.id]
      : _adVariant;

  function handleLike() {
    setReactionState((c) =>
      c === REACTION.LIKED ? REACTION.NEUTRAL : REACTION.LIKED
    );
  }

  function handleDislike() {
    setReactionState((c) =>
      c === REACTION.DISLIKED ? REACTION.NEUTRAL : REACTION.DISLIKED
    );
  }
  ...
  return (
    ...
    >
      ...
      {!mock.is && (
        <motion.div
          className="px-[3.75em] overflow-hidden"
          initial={{ height: 0 }}
          onAnimationEnd={() => {
            headingRef.current &&
          ...
          <ol className="list-decimal">
            <li className="pl-1">
              <span className="text-[1.05em] font-medium">
                <strong>Ad Orientation</strong>
              </span>
              <p className="mt-[0.3em] text-[1em] font-diatype opacity-60 leading-[1.5em]">
                {adVariant.adOrientation}
              </p>
            </li>
            {adVariant.imageDescription && (
              <li className="pl-1 relative">
                <p className="text-[1.05em] font-medium z-10 relative">
                  <strong>Image Description</strong>
                </p>
                <p className="mt-[0.3em] mb-[1em] text-[1em] opacity-60 relative z-10 leading-[1.5em]">
                  {adVariant.imageDescription}
                </p>
            ...
            <li className="pl-1">
              ...
              <p className="mt-[0.3em] mb-[1em] text-[1em] opacity-60 leading-[1.5em]">
                {adVariant.rationale}
              </p>
            </li>
          </ol>
...
export const FacebookAdVariantMini: FC<FacebookAdVariantProps> = ({
  adVariant: _adVariant,
  noExpand,
  className,
  ...props
  ...
  let lottieAnimationJSX = (
    <div className="w-full aspect-square flex flex-col justify-center items-center">
      <Lottie
        ...
        loop={true}
      />
      <h6 className="text-white text-opacity-60 text-[7.25px] text-center px-5 leading-normal">
        Creating your ad variant image to make your brand shine, one pixel at a
        time.
      </h6>
    </div>
  ...
  const imageContainerJSX = adVariant.imageUrl ? (
    <Image1
      width={600}
      height={100}
      className="mb-[2.5px] w-full"
      src={adVariant.imageUrl ? adVariant.imageUrl : dummyImage}
      alt="Ad Image"
    />
  ) : (
    lottieAnimationJSX
  ...
  return (
    <div
      key={adVariant.oneLiner}
      className={
        "ad-variant text-[7px] md:text-[35px] !p-0 " + (className ?? "")
      }
      {...props}
    >
      <div
        className={
          "flex justify-between items-center mb-[2.1px] px-[4px] pt-[3px]"
        }
      >
      ...
      <div className="mb-[7px] px-[7px]">
        <span className="text-[6.65px] leading-[10.5px] line-clamp-4 text-ellipsis">
          {adVariant.text}
        </span>
      </div>
      <div>
      ...
      <div
        className={
          "flex justify-between gap-[5.6px] items-center px-[7px] mt-[7px]"
        }
      >
        <span className={"text-[8.75px] leading-[9.1px]"}>
          {adVariant.oneLiner}
        </span>
        <div className="flex-shrink-0">
          <span className="cursor-pointer rounded bg-gray-700 px-[4.2px] py-[3.5px] text-[7px]">
            Learn More
          </span>
        </div>
      </div>
